...
void main() {
  ...
  for (var p in partidos) {
    // 3 PL, 1 Base
    for(int i=0; i<3; i++) {
        gallos.add(Gallo(id: galloId++, partidoId: p.id, pesoGramos: 2000.0 + (i * 100), esBase: false, anillo: 'PL${i}'));
    }
    gallos.add(Gallo(id: galloId++, partidoId: p.id, pesoGramos: 2100, esBase: true, anillo: 'BASE'));
  }
